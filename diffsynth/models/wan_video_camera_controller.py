...
def process_pose_file(cam_params, width=672, height=384, original_pose_width=1280, original_pose_height=720, device='cpu', return_poses=False, is_traj=False):
    if return_poses:
        ...
        b, f, _ = cam_params.shape
        default_value = [0.532139961, 0.946026558, 0.5, 0.5]
        Ks = torch.tensor(default_value).repeat(b, f, 1).to(device=cam_params.device, dtype=cam_params.dtype)
        ...
        c2w = rearrange(cam_params, 'b f (i j) -> b f i j', i=3, j=4)
